...
Public Const Tool_Name = "sgRNA Annotator v1.0"
...
'            End If
        ...
        If GenBank_File_Path <> "" Then
            Call Print_Log(0, "Annotate_sgRNAs_Browse", "Selected file: " & MyFiles, "Good")
        Else
...
'
'    Tempo_File_Address = ActiveWorkbook.Path & "/Library/" & Left(Gene_Name, Len(Gene_Name) - 1) & ".gb"
'
'
...
Sub Update_Dual_sgRNA_combinations()
    
    On Error GoTo Error_Handler
    
    Call Duals_Housekeeping
    Call Update_Duals
...
Error_Handler:
    If Err.Number <> 0 Then
        User_Notification = Err.Description
        On Error Resume Next
        Call Print_Log(0, "Update_Dual_sgRNA_combinations", User_Notification, "Bad")
        MsgBox "Update_Dual_sgRNA_combinations failed! Please check the Log worksheet.", vbInformation, Tool_Name
    Else
        Call Print_Log(0, "Update_Dual_sgRNA_combinations", "Done!", "Good")
        MsgBox "Done!", vbInformation, Tool_Name
    End If
